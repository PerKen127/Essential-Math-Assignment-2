...
#pragma region obstacle 1
	...
	//Setup static START Wall
	{
		...
		tempPhsBody = PhysicsBody(entity, tempBody, float(tempSpr.GetWidth() - shrinkX), float(tempSpr.GetHeight() - shrinkY), vec2(0.f, 0.f), false, OBJECTS, GROUND | ENVIRONMENT | PLAYER | ENEMY);
		tempPhsBody.SetColor(vec4(0.f, 1.f, 0.f, 0.3f));
		tempPhsBody.SetRotationAngleDeg(90.f);
	}
	//Ball
...
#pragma region obstacle 2
	...
	//Setup higher platform wall
	{
		...
		//Sets up components
		std::string fileName = "boxSprite.jpg";
		ECS::GetComponent<Sprite>(entity).LoadSprite(fileName, 60, 15);
		ECS::GetComponent<Transform>(entity).SetPosition(vec3(30.f, -20.f, 2.f));
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(672.5f), float32(55.f));
	...
	//Setup trigger platform left wall
	{
		//Creates entity
		auto entity = ECS::CreateEntity();
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(920.f), float32(-30.f));
	...
	//Setup trigger platform right wall
	{
		//Creates entity
		auto entity = ECS::CreateEntity();
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(975.f), float32(-30.f));
	...
	//Setup trigger platform
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(950.f), float32(-50.f));
	...
	//Setup Blocker
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(1150.f), float32(60.f));
	...
	//Setup trigger
	{
		...
		ECS::GetComponent<Trigger*>(entity) = new DestroyTrigger();
		ECS::GetComponent<Trigger*>(entity)->SetTriggerEntity(entity);
		ECS::GetComponent<Trigger*>(entity)->AddTargetEntity(blocker2);
		ECS::GetComponent<Sprite>(entity).SetTransparency(0.f);
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(947.5f), float32(-25.f));
	...
	//Setup last platform
	{
		...
		//Sets up components
		std::string fileName = "boxSprite.jpg";
		ECS::GetComponent<Sprite>(entity).LoadSprite(fileName, 350, 10);
		ECS::GetComponent<Transform>(entity).SetPosition(vec3(30.f, -20.f, 2.f));
	...
	//Setup box
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_dynamicBody;
		tempDef.position.Set(float32(725.f), float32(70.f));
...
#pragma region ending
	//Setup last platform
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(2150.f), float32(-10.f));
	...
	//Setup end wall
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(2325.f), float32(15.f));
...
void PhysicsPlayground::KeyboardHold()
{
	...
	{
		player.ScaleBody(4. * Timer::deltaTime, 0);
	}
